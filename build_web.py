...
# The source bar has two modes, switched by body.has-board.
#
# Landing: a heading, one sentence, and the bar laid out in full: the status
# card, the folder button, and the rarer controls inline.
#
# Board: the heading and sentence are gone, the bar is a slim strip above the
# masthead with the card, Update, and a "more" menu that holds everything
# else. Same elements, same handlers; only the CSS changes.
BANNER = r"""<style>
[hidden]{display:none!important}
/* The board gives every section content-visibility:auto, which implies paint
   containment and would clip the dropdown to the strip. Not here. */
.landing{max-width:1180px;margin:0 auto;padding:18px 24px 0;content-visibility:visible;contain:none}
...
.src .btn{font:inherit;font-size:13px;border:1px solid var(--rule);border-radius:4px;padding:7px 12px;background:var(--raised);
  color:var(--ink);cursor:pointer;line-height:1.2;white-space:nowrap}
...
.src .chip{display:inline-flex;align-items:center;gap:6px;font:inherit;font-size:12px;border:1px solid var(--rule);border-radius:999px;
  padding:4px 10px;background:var(--surface);color:var(--ink-2);cursor:pointer;white-space:nowrap}
...
.src .more{font-size:12px;color:var(--ink-3);white-space:nowrap}
...
.src details.help{font-size:12px;color:var(--ink-2)}
.src details.help summary{cursor:pointer;color:var(--ink-3);text-decoration:underline;list-style:none;white-space:nowrap}
.src details.help summary::-webkit-details-marker{display:none}
.src details.help[open]{flex-basis:100%;background:var(--surface);border:1px solid var(--rule);border-radius:6px;padding:10px 14px}
.src details.help[open] summary{margin-bottom:6px;text-decoration:none;color:var(--ink)}
.src details.help p{margin:4px 0;max-width:none}
.src details.help code{font-family:"IBM Plex Mono",ui-monospace,monospace;font-size:11.5px;background:var(--raised);padding:2px 6px;border-radius:3px;user-select:all}
.src details.help .muted{color:var(--ink-3)}
...
/* The "more" menu: inline on the landing, a dropdown on the board. */
.src .menu{display:contents}
.src .menu > .toggle{display:none}
.src .menu-body{display:contents}
body:not(.has-board) #updateBtn:disabled{display:none}

body:not(.has-board) .wrap{display:none}
body.has-board .landing h1, body.has-board .landing .lede{display:none}
body.has-board .landing{padding:10px 24px 0;position:relative;z-index:60}
body.has-board .src{margin-top:0;gap:8px}
body.has-board .src .card{padding:5px 10px;min-width:220px;flex:0 1 460px}
body.has-board .src .card .txt b{font-size:13px}
body.has-board .src .btn{padding:5px 11px;font-size:12.5px}
body.has-board .src .menu{display:block;position:relative;margin-left:auto}
body.has-board .src .menu > .toggle{display:inline-block}
body.has-board .src .menu-body{display:none;position:absolute;right:0;top:calc(100% + 6px);z-index:40;min-width:260px;
  background:var(--surface);border:1px solid var(--rule);border-radius:6px;padding:8px;box-shadow:0 8px 24px rgba(0,0,0,.18);
  flex-direction:column;align-items:stretch;gap:6px}
body.has-board .src .menu.open .menu-body{display:flex}
body.has-board .src .menu-body .btn, body.has-board .src .menu-body .chip{text-align:left;justify-content:flex-start;border-radius:4px;width:100%}
body.has-board .src .menu-body details.help[open]{flex-basis:auto;padding:8px 10px}
body.has-board .src .menu-body .more{margin:2px 4px 0}
</style>
<section class="landing">
  ...
  <div class="src">
    ...
    <div class="menu" id="srcMenu">
      <button type="button" class="btn toggle" id="menuBtn" aria-haspopup="true" aria-expanded="false" title="Change the folder, pick one file, load the game text, and more">More &#9662;</button>
      <div class="menu-body">

...
- **Approving a browser.** Every write needs `Authorization: Bearer <token>`, a token
  ...
  40 characters ("a browser" when nothing is left; a program with no origin then shows no
  name); opening the map over the popup counts
...
## The game members it uses (build 3682)
...
| `HudConfirm.Show(LanguageChangeEventDataHolder, LanguageChangeEventDataHolder, Action, Action, string, string, bool, bool)`, `HudConfirm.isOpen`, `onShow`, `onClose`; `Localizor.LocalizorManager.Localize(key, args)`; `HudConfirmUi._onConfirmAction`, `container` and `showInFullMenu` (**private**, reflection: our popup, and the one on screen), `UI.Smartphone.FullMenu.IsOpen`, `CityMap.IsOpen`; `UnityEngine.PlayerPrefs` (not the game's own `PlayerPrefs` class) | approving a browser. `Show` confirms unseen when no popup UI is registered (`onShow` null) and drops the call when one is already open, so the mod checks both first. No popup over the city map; `LocalizorManager.IsLocalizedKey` (a browser name must not expand to game text) |
...
| `ImportPartnership` fields and `NextDeliveryTotal`, `GetDiscount`, static `GetItemAmountOrderedThisWeek`; `ImportProduct.Price`; `DeliveryHelper.CanModifyContract`, `GetNextDeliveryDay`, `IsLockPeriod`, `ShouldLimitImporterMaxAmount`, `AreWholesaleAndImportLimitsDisabled`; `ProductMarketHelper.IsProductInMarketEvent`; `EmployeeHelper.GetEmployeeById(id, false)` | imports. `ImportPartnership.GetMaxOrderAmountPerImporter` is **private static**: by reflection, with its build-3682 body (the item's `maxOrderAmountPerImporter`, ×0.66 rounded in a shortage) as the fallback |

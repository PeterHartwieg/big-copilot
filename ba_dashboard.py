...
VERIFIED_BUILD = 3682  # the game build every number here was last checked against
...
def _crew(crew: list) -> list:
    ...
    for person in sorted(crew, key=lambda c: (c["role"], -c["level"])):
        entry = roles.setdefault(
            person["role"],
            {"role": person["role"], "count": 0, "daily": 0.0, "skill": 0, "absent": 0},
        )
        entry["count"] += 1
        entry["absent"] += 1 if person.get("absent") else 0
...
def _import_setting(supply: dict) -> dict:
    ...
    level of the contract that holds it, named by `levelImporter` and
    `levelId` (_import_level); `levelName` is how the page names it, the
    importer, with the contract's number where that importer has several. `plain` is what plain contracts on the line
    bring a week: `plainBefore` of it is delivered before that level and
    counts toward it, `plainAfter` comes after it, on top. `pass` is the
    counted contracts in delivery order and `levelAt` the named one's place in
    it, so the page can replay the pass with another level (importLevelFor).
    `arrivedLastWeek` sums every contract's amountOrderedLastWeek, paused ones
    too, since those goods did arrive. `contracts` lists each contract on the
    line in the order the game delivers them.
    """
    ...
    return {
        ...
        "plainBefore": supply.get("plainBefore", 0),
        "levelImporter": supply.get("levelImporter"),
        "levelName": supply.get("levelName", supply.get("levelImporter")),
        "levelId": supply.get("levelId"),
        "levelAt": supply.get("levelAt"),
        "pass": [{"amount": d["amount"], "smart": d["smart"]} for d in supply.get("pass", [])],
...
def _import_level(drops: list) -> dict:
    """Which Smart Delivery contract holds a line's level, and the plain around it.

    `drops` is in delivery order, as _import_drop() walks them. The contract
    named (`at`, its index) is the last Smart Delivery one that still brings
    something into an empty depot: a later level finds the depot already at
    or above it and brings nothing. Where none brings anything, because plain
    amounts delivered first already pass every level, the last one is named.
    `plainBefore` is the plain amount delivered before it, which counts toward
    its level (the top-up is only what is missing); `plainAfter` is delivered
    after it and comes on top. A line with no level has `smart` False.
    """
    ...
    for i, drop in enumerate(drops):
        ...
        stock += brought
    if last is None:
        return {"smart": False, "level": None, "at": None, "plainBefore": 0, "plainAfter": 0}
    at = holds if holds is not None else last
    plain = lambda part: sum(d["amount"] for d in part if not d["smart"])
    return {"smart": True, "level": drops[at]["amount"], "at": at,
            "plainBefore": plain(drops[:at]), "plainAfter": plain(drops[at + 1:])}


def _import_pass(drops: list, at: int, level) -> float:
    """What one delivery brings into an empty depot with the contract at `at`
    set to `level`: the pass _import_drop() makes, replayed."""
    return _import_drop(0, [dict(d, amount=level) if i == at else d for i, d in enumerate(drops)])


def _level_for(drops: list, at: int, need) -> int:
    """The smallest level, in hundreds, for the contract at `at` whose pass
    brings `need`.

    Found by replaying the pass, not by arithmetic on the plain amounts: a
    plain amount delivered before the level counts toward it, one after it
    comes on top, and a later level may take over. The pass only grows with
    the level and brings at least the level itself, so `need` rounded up to
    the hundred always suffices and a halving search finds the least.
    """
    lo, hi = 0, math.ceil(max(need, 0) / 100)
    while lo < hi:
        mid = (lo + hi) // 2
        if _import_pass(drops, at, mid * 100) >= need:
            hi = mid
        else:
            lo = mid + 1
    return lo * 100
...
def _raise_import(supply: dict, need) -> int:
    """What to raise a line's import to so a week brings `need`: the named
    level, replayed through the pass, for Smart Delivery; the week rounded up
    to the hundred for a plain order. A Smart Delivery line with no pass (a
    hand-built one) takes off what comes after the level, as the page does
    (importSetting, importRaise)."""
    if supply.get("smart") and supply.get("pass") and supply.get("levelAt") is not None:
        return _level_for(supply["pass"], supply["levelAt"], need)
    if supply.get("smart"):
        return _ceil_hundred(max(0, need - supply.get("plainAfter", 0)))
...
def _supply(
    ...
    for order, partnership in enumerate(partnerships):
        ...
        for product in save.items(partnership["products"]):
            ...
            drop = {"day": arrives, "amount": amount, "smart": smart, "rank": rank,
                    "importer": who, "id": partnership.get("id")}
            if supplying:
    ...
    for line, supply in imports.items():
        ...
        holds = _import_level(lead)
        supply["smart"], supply["target"] = holds["smart"], holds["level"]
        supply["plainAfter"], supply["plainBefore"] = holds["plainAfter"], holds["plainBefore"]
        supply["pass"], supply["levelAt"] = lead, holds["at"]
        named = lead[holds["at"]] if holds["smart"] else {}
        supply["levelImporter"], supply["levelId"] = named.get("importer"), named.get("id")
        supply["plain"] = sum(d["amount"] for d in lead if not d["smart"])
        supply["contracts"] = _contract_list(contracts_at[line])
        supply["levelName"] = _level_name(
            supply["contracts"], named.get("importer"), named["rank"][1] if named else None)
            ...
            import_rows.append(
                {
                    ...
                    **_import_setting(supply),
                    ...
                    "catchUp": catch_up,
                    "coverageUntil": schedule["until"] if schedule else supply["arrives"],
...
def _depot_flow(flow: dict, index: dict, machines: dict, depot_need: dict) -> tuple:
    ...
    # A Smart Delivery line among them names the contract to set, as a live
    # one does: the first Smart Delivery contract in delivery order, an
    # active one first, replayed over the contracts of its own group.
    for (depot, slug), contracts in flow.get("configured", {}).items():
        if depot not in index:
            continue
        smart = [c for c in contracts if c["smart"]]
        named = next((c for c in smart if c["active"]), smart[0] if smart else None)
        group = [c for c in contracts if named is not None and c["active"] == named["active"]]
        depots[index[depot]][slug] = {
            "weekly": 0, "pausedWeekly": 0, "zeroOnly": True,
            "smart": named is not None, "target": named["amount"] if named else None,
            "plain": 0, "plainAfter": 0, "plainBefore": 0,
            "levelImporter": named["importer"] if named else None,
            "levelId": named["id"] if named else None,
            "levelName": _level_name(contracts, named["importer"], named["order"]) if named else None,
            "levelAt": group.index(named) if named else None,
...
def _factories(
    ...
    for key, counter in machines.items():
        ...
        for slug, row in needs.items():
            ...
            own_import = flow["imports"].get((key, slug))
            direct_weekly = own_import["weekly"] if own_import else 0
            mixed = bool(own_import and target and source)
            direct_need = min(direct_weekly, row["perDay"] * 7) if mixed else row["perDay"] * 7 if own_import else 0
            ...
            row["factoryImportSite"] = index[key] if own_import else None
            ...
            row["depotStock"] = held(source, slug) if source and not direct else 0
            ...
            if source:
                depot_need[(source, slug)] += warehouse_need
    ...
    for site in sites:
        for row in site["needs"]:
            ...
            row["importPlainBefore"] = supply.get("plainBefore", 0) if row["importSmart"] else 0
            row["importLevelImporter"] = supply.get("levelImporter") if row["importSmart"] else None
            row["importLevelName"] = (
                supply.get("levelName", supply.get("levelImporter")) if row["importSmart"] else None)
            ...
            if row["importPaused"]:
                status, level = "paused", "critical"
            elif row["directImport"]:
                if row["importFit"] in ("short", "tight"):
                    status, level = "import", "critical" if row["importFit"] == "short" else "warn"
                    if row["importFit"] == "short":
                        row["raiseImport"] = _raise_import(supply, weekly_need)
                else:
                    status, level = "ok", "ok"
            elif not row["target"]:
                status, level = "unplanned", "critical"
            elif row["target"] < row["dailyNeed"] * (1 - FEED_SLACK):
                status, level = "target", "critical"
                row["raiseTarget"] = _ceil_hundred(row["dailyNeed"])
...
def _fill_by_exchange(shifts: list, pool: list, state: dict, rostered: set) -> int:
    ...
    the first days of the week, and the last days are left to people who may
    take one entry a day: two registers around the clock come out with seven
    people and three entries still open, where seven people can work all 28.
...
def _staffing(
    ...
    Two passes. Every site's demand plan first, in site order, each taking
    whom it uses off the bench; then every site's full-cover plan, in the same
    ...
    def live(entry):
        ...
        if site is None:
            return 1
        grid = site["grid"]
        opens = [wd for wd in range(7) if grid["open"][wd]]
    ...
    for index, (business, site) in sorted(enumerate(sites), key=lambda e: (live(e), e[0])):
        ...
        site_bench = [p for p in everyone_on_bench if p["id"] in mine or p["id"] in free]
...
# enough for the demand plan (Peter, 23 September 2026): nine covers every
# weekday once and two of them twice, a week and a bit of game time rather than
# the fortnight HOUR_WEEKS_THIN would take. Not nine in a row: a nightclub shut
# every Monday still completes on its ninth day, and its Mondays are no demand.
DEMAND_RUN_DAYS = 9


def _days_open(save: Save, building: dict) -> int:
    """Finished days since the shop first opened, today's unfinished day left out.

    The first open day is the earliest day on file with at least one hour
...
def _run_measured(grid: dict, run: int, daily: list | None = None) -> dict:
    ...
    a thin weekday off another through the game's day curve, or not at all; an
    hour with no report it reads off another weekday the same way. Nine days
    since the shop first opened are what Peter decided is enough (23 September
...
def _unmeasured(grid: dict, run: int) -> list:
    """Hours the current schedule opens that no report has measured, per weekday.

    Only for a shop with complete data, where such an hour counts as no
    customers: it is the missing data the page points at, so a player who
    measured 8 to 22 and now opens around the clock knows the night is a guess
    of nobody until the reports arrive.
    """
    if run < DEMAND_RUN_DAYS:
    ...
    return [
        sorted(
            ...
            if grid["customers"][wd][hour] is None
        )
        for wd in range(7)
    ]
...
def _demand_data_complete(run, week, grid, in_game) -> bool:
    ...
    - the demand data is complete: at least DEMAND_RUN_DAYS finished days since
      the shop first opened (_days_open()), read from the days as they were
      staffed, whoever was on;
...
def _full_cover_in_game(current: list, stations: list, slots: list) -> bool:
    """Whether the schedule in the game is full cover of the shop's own hours.

    Every serving station staffed every hour the shop is open, whatever those
    hours are (Peter, 23 September 2026: the player may keep a shorter day, and
    the plan counts its shut hours as no demand). Nothing less inside them: an
    hour a register stands empty is an hour whose customers were turned away by
    the schedule rather than by demand. A site with no serving station, or one
    open no hour at all, has no demand to test, so it never is one.
    """
    opened = {
        (wd, hour)
        for wd in range(7)
        for start, end in slots[wd]
        for hour in range(max(0, start), min(24, end))
    }
    if not stations or not opened:
        return False
    ...
    return all(opened <= hours for hours in covered.values())
...
def _finish_site(site, full, names, people) -> dict:
    ...
    cost = {
        "current": money(_current_cost(current, site["wage"])),
        "currentCover": money(_current_cost(
            {"list": [r for r in current["list"] if r["kind"] != "serve"]}, site["wage"])),
    }
    ...
    return {
        ...
        **_plan_fields(week, table, names, people, cost),
        ...
        "fullCover": {
            ...
            "inGame": in_game,
            # Finished days since the shop first opened, and the days that make
            # the demand data complete: the test's progress.
...
def _plan(
    ...
    """
    # A fee link — (fees-haircut), say — charges for work done, never for a
    ...
    for key, text in names.locale.items():
        match = _BUSINESS_HELP_RE.match(key)
        if match:
            pages[match.group(1)] = text
            services.update("ba:itemname_" + s for _n, s in fee_re.findall(text))

    catalogue_out = {}
    for kind, items in catalogue.items():
        sold = set(items)
        text = pages.get(kind)
        header = _SELLS_HEADER_RE.search(text) if text else None
        sells = text[header.end() :].lstrip("\n").split("\n\n", 1)[0] if header else ""
        before = text[: header.start()] if header else (text or "")
        # What the type charges for is its service line: recorded for the page
        # to explain, and kept out of the products the planner can stock.
        charged = {"ba:itemname_" + s for _n, s in fee_re.findall(sells)}
        kind_services = sorted(charged | (sold & services))
        physical = sold - services
        if kind_services:
            # A service business still runs on goods — the hairdresser's chair
            # is stocked with a haircare product — and the required-furniture
    ...
    for kind, products in mine.items():
        # Only physical lines count as trade the planner can plan; what the
    ...
    # has a number in the box. One product can sit on several contracts, so the
    ...
    for order, partnership in enumerate(partnerships):
        ...
        for product in save.items(partnership["products"]):
            ...
            drops[(product["itemName"], warehouse)]["active" if active else "paused"].append(
                {"amount": amount, "smart": smart, "rank": ranks[order], "importer": who}
            )
...
def _finding(
    ...
    *,
    ...
    `key` is the business's site key (its address) when the finding is about
    one business; a synthetic site such as "3 shops" has none. The id hashes
    ...
    for row in supply["imports"]:
        ...
        if row["reason"] == "paused":
            ...
            note(
                ...
                "order",
                (
                    f"{row['item']}: {_smart_words(row['target'], row['plainAfter'], row['plainBefore'])} against a "
                    if row.get("smart")
...
def _feed_notes(businesses: list, factories: dict, silent: set) -> list:
    ...
    for site in factories.get("sites", []):
        ...
        for row in site["needs"]:
            ...
            if row["status"] in ("import", "noimport", "paused"):
            ...
            status = row["status"]
            if status == "paused":
                text = f"{row['item']} import is paused; resume the contract supplying {where}"
...
def _condense(found: list, gate: float) -> dict:
    # One site is one key; a synthetic site ("3 shops") has none and is its name.
    buckets = collections.OrderedDict()
    for item in found:
        buckets.setdefault((item["siteKey"] or item["site"], item["group"]), []).append(item)

    out = []
    for (where, group), rows in buckets.items():
...
button.unname{
  ...
  border:1px solid var(--rule);border-radius:5px;padding:2px 5px;margin-left:4px;cursor:pointer;
  display:inline-flex;align-items:center;vertical-align:middle;
}
button.unname svg{width:11px;height:11px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
...
/* The balls may roll behind the clock, so it sits above them. Each line of
   ...
   through. A halo that hugs each letter does the real work of keeping the
   small grey lines legible over the green. */
.clock{margin-left:auto;text-align:right;cursor:default;position:relative;z-index:7}
.clock > b,.clock > small{position:relative;
  text-shadow:0 0 1px var(--ground),0 0 2px var(--ground),0 0 4px var(--ground),0 0 7px var(--ground)}
.clock > b::before,.clock > small::before{content:"";position:absolute;inset:-2px -6px;z-index:-1;pointer-events:none;
...
/* The real clock also carries staff counts. Each line is only as wide as its
...
button.ibtn{padding:0;font:inherit;appearance:none;-webkit-appearance:none}
...
/* Weekly imports: the unit beside a figure, the Set to box and its reset, and
   the contracts listed under a material that has more than one. */
#importPlan .imp-unit{font:400 11px Archivo,sans-serif;color:var(--ink-3)}
#importPlan .imp-set{display:inline-flex;align-items:center;gap:2px;vertical-align:middle}
#importPlan .imp-in{width:6.5em;box-sizing:border-box;font:inherit;text-align:right;padding:3px 6px;border:1px solid var(--rule);border-radius:5px;background:var(--surface);color:var(--ink);-moz-appearance:textfield;appearance:textfield}
#importPlan .imp-in::-webkit-inner-spin-button,#importPlan .imp-in::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}
#importPlan .imp-in:focus-visible{outline:2px solid var(--accent);outline-offset:1px}
#importPlan tr.imp-changed .imp-in{border-color:var(--accent);color:var(--accent);font-weight:600}
#importPlan .imp-reset{display:inline-grid;place-items:center;width:22px;height:22px;padding:0;border:0;border-radius:4px;background:none;color:var(--ink-3);cursor:pointer}
#importPlan .imp-reset:hover,#importPlan .imp-reset:focus-visible{color:var(--accent)}
#importPlan .imp-reset svg{width:13px;height:13px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
#importPlan td.imp-to > .chip{margin-left:6px}
#importPlan .imp-contracts{min-width:12rem}
#importPlan .imp-contracts > span{display:block}
...
.step .machines em{font:500 11px/1 "IBM Plex Mono",monospace;font-style:normal;color:var(--ink-3);align-self:center}
.covers .sub{white-space:nowrap}
.covers .sub .chip{vertical-align:middle}
...
/* open hours with no report yet: one line, and a mark on the need strip */
.sp-unmline{display:flex;align-items:center;gap:8px;margin:0 0 14px;font-size:12.5px;line-height:1.45;color:var(--ink-2)}
.sp-unmline .sp-i{color:var(--warn)}
.sp-unmline b{color:var(--ink);font-weight:600}
.sp-unmh{grid-row:1;align-self:end;height:6px;border-bottom:2px dotted var(--warn);cursor:default}
    ...
    <section class="sec rv" id="alertSection">
      ...
      <div id="alertHead">
        <button type="button" class="ibtn tr" id="alertKindsToggle" aria-expanded="false"
          data-tip="Which kinds of finding make the list"><svg viewBox="0 0 24 24"><path d="M4 7h10M18 7h2M4 17h4M12 17h8"></path><circle cx="16" cy="7" r="2"></circle><circle cx="10" cy="17" r="2"></circle></svg></button>
      </div>
  ...
  <div class="page" id="pageSupply" hidden>
    ...
    <section class="sec rv" id="secLogistics" data-sub="orders">
      <details class="order-checklist" id="orderChecklist">
        <summary id="orderChecklistTitle">Change checklist</summary>
        <div id="orderChecklistBody"></div>
      </details>
  ...
  <div class="page" id="pageGrowth" hidden>
    ...
    <section class="sec rv" id="secIngredients" data-sub="plan">
      ...
      <table class="ingtable" id="ingTable">
        <thead><tr><th>Ingredient</th><th class="l">Used by</th><th>Per day</th><th>Per week</th><th>Company target</th>
...
const SUPPLY_VIEWS = {
  shops: {
    ...
    verdict: rows => {
      ...
      const unplanned = rows.filter(stockUnplanned).length;
      const bad = rows.filter(r => !stockUnplanned(r) && r.level !== "ok").length;
      const clear = rows.length - unplanned - bad;
      const problems = [
        unplanned ? `${unplanned} ${unplanned===1?"shelf has":"shelves have"} no top-up plan` : "",
        bad ? `${bad} ${bad===1?"shelf does not clear its":"shelves do not clear their"} top-up` : "",
      ].filter(Boolean);
  ...
  feed: {
    ...
    verdict: rows => {
      ...
      const problems = [
        bad ? `${bad} input${bad===1?" is":"s are"} not fed as the machines need${
          unplanned ? ` (${unplanned} ${unplanned===1?"has":"have"} no top-up plan)` : ""}` : "",
    ...
    row: r => {
      ...
      const change = {
        paused: () => `${chip("bad", "import paused")} resume the import contract`,
      ...
      <td>${r.importWeekly !== null ? r.importWeekly.toLocaleString() : "—"}${
          !r.directImport && r.directWeekly ? `<span class="sub">plus ${r.directWeekly.toLocaleString()} direct</span>` : ""}${
          r.depotNeed && r.depotNeed !== r.perWeek
          ? `<span class="sub"> ${r.directWeekly ? "warehouse share" : "all factories"} ${r.depotNeed.toLocaleString()}</span>` : ""}</td>
...
/* The delivery pass, replayed: what one delivery brings into an empty depot
   with the contract at `at` set to `level` (_import_pass). `pass` is the
   line's counted contracts in delivery order, as {amount, smart}. */
function importPassDelivers(pass, at, level){
  let stock = 0;
  pass.forEach((d, i) => {
    const amount = i === at ? level : d.amount;
    stock += d.smart ? Math.max(0, amount - Math.max(stock, 0)) : amount;
  });
  return stock;
}
/* The smallest level, in hundreds, for the contract at `at` whose pass brings
   `need` (_level_for). A plain amount before the level counts toward it, one
   after it comes on top, and a later level may take over, so the pass is
   replayed rather than the plain amounts subtracted. The pass only grows with
   the level and brings at least the level, so `need` rounded up suffices. */
function importLevelFor(pass, at, need){
  let lo = 0, hi = Math.ceil(Math.max(need, 0) / 100);
  while(lo < hi){
    const mid = Math.floor((lo + hi) / 2);
    if(importPassDelivers(pass, at, mid * 100) >= need) hi = mid; else lo = mid + 1;
  }
  return lo * 100;
}
/* What to raise a factory input's import to: the named Smart Delivery level,
   replayed, or the week rounded up to the hundred for a plain order. */
const importRaise = n => n.importSmart && (n.importPass || []).length && Number.isInteger(n.importLevelAt)
  ? importLevelFor(n.importPass, n.importLevelAt, n.depotNeed)
  : Math.ceil(Math.max(0, n.depotNeed - (n.importSmart ? n.importPlainAfter || 0 : 0)) / 100) * 100;
...
function feedVerdict(n){
  ...
  if(n.importPaused){ status = "paused"; level = "critical"; }
    ...
    if(n.importFit === "short"){
      status = "import"; level = "critical"; n.raiseImport = importRaise(n);
  ...
  else if(n.target < (n.dailyNeed ?? n.perDay) * 0.98){ status = "target"; level = "critical"; n.raiseTarget = ceil100(n.dailyNeed ?? n.perDay); }
...
function feedRoute(site, n, view, held){
  ...
  n.directNeed = mixed ? Math.min(own.weekly, n.perWeek) : own ? n.perWeek : 0;
  n.warehouseNeed = source !== null && (mixed || !own) ? Math.max(n.perWeek - n.directNeed, 0) : 0;
  n.directImport = !!(own && !n.warehouseNeed);
  ...
  n.importPlainBefore = supply && supply.smart ? supply.plainBefore || 0 : 0;
  n.importLevelImporter = supply && supply.smart ? supply.levelImporter ?? null : null;
...
function factoryView(){
  ...
  if(touched.size){
    ...
    view.sites.forEach(s => {
      s.needs.forEach(n => {
        feedRoute(s, n, view, held);
        for(const [source, week] of [[n.factoryImportSite, n.directNeed], [n.from, n.warehouseNeed]]){
          if(source === null) continue;
          const k = source + "|" + n.slug; depotNeed[k] = (depotNeed[k] || 0) + week;
        }
      });
      s.lines.forEach(l => {
...
function drawKpis(){
  ...
  /* A tile is a number, one chip, a short sub line and the whole daily history
     as a sparkline. The sentence the tile used to print is the chip's tooltip.
...
/* The business a finding is about: by its site key, which survives a rename
   ...
   no key and resolves to nothing, so its row never wears another shop's pill
   and never opens one. The name is only a fallback for a row with no key
   field at all. */
function alertSite(a){
  if(a.siteKey) return D.businesses.find(x => x.key === a.siteKey) || null;
...
function drawChart(){
  ...
  /* A save on its first day has no finished day to plot yet. The web shell
     reloads saves in place, so the last save's chart is cleared rather than
     left standing. */
  if(!n){
    chartRows = [];
...
function drawPortfolio(){
  ...
  const body = [];
  const chains = (D.chains || []).slice();
  if(sorter) chains.sort((a, z) => (v.chainKey(a, sortKey) - v.chainKey(z, sortKey)) * sortDir);
  chains.forEach(c => {
...
/* Hours, not blocks: the week as the game counts it, on both sides of every
   comparison here. How many blocks those hours come in is a separate question
   -- it is the dragging, not the week. */
const spHoursOf = list => (list || []).reduce((n, s) => n + Math.max(0, s.t - s.f), 0);
/* The hours a plan asks for: the ones somebody can be put on, or -- on a shop
   whose every block waits on a hire -- the hours it wants and has nobody for.
   The two are never added, because a week half of which cannot be set yet is
   not a week the player can act on. */
const spPlanHours = row => {
  const drawn = (row.shifts || []).filter(s => spDrawn(row, s));
  const on = drawn.filter(s => !spNobody(s.p));
  return spHoursOf(on.length ? on : drawn);
};
...
/* What a skill is called on the page. A serving role has its own label; a
   ...
   the station itself. */
const spSkillLabel = (row, stations, skill) =>
  ((row.roles || []).find(r => r.skill === skill) || {}).label
  || ((stations || []).find(st => st.skill === skill) || {}).name
  || String(skill).split("_").pop();
...
/* --- the two plans ----------------------------------------------------------
   ...
   pick then lasts as long as the page does. A pick made on this page wins over
...
function spHourRanges(hours, wrap = true){
  ...
  for(let k = 0; k < 24; k++){
    const h = (start + k) % 24;
    if(!on.has(h)) continue;
    const last = runs[runs.length - 1];
    if(last && last[1] % 24 === h) last[1] = h + 1;
...
function spRosterBlock(b){
  ...
  /* The same two sides in hours, which is what the player is really setting:
     a schedule is dragged out in hours and days, and the block count beside
     them is only how much of that dragging the plan saves. */
  const drawnHere = (row.shifts || []).filter(s => spDrawn(row, s));
  const planHours = spHoursOf(drawnHere.filter(s => !spNobody(s.p)));
  /* Counted off the rows the counts above are counted off -- the whole
     schedule, or its cleaning and security half -- so the hours and the
     blocks beside them are about the same week. */
  const nowList = (row.current || {}).list || [];
  const againstHours = spHoursOf(c.cover ? nowList.filter(s => !!s.k) : nowList);
  ...
  const step = (label, sub, tip) => `<button type="button" class="sp-step"${
    tip ? ` data-tip="${attr(tip)}"` : ""}><span class="sp-box">${
    spIcon("tick")}</span>${label}<small>${sub}</small></button>`;
  const posts = spPlanPosts(row);
  /* Which station each hire is for. "3 posts" named nothing the player could
     go and look at, and the hires are sized at the game's full-time week
  ...
  imports.filter(take).forEach(r => {
    ...
    const read = r.paused ? `Import <b>paused</b>; <b>${spNum(r.cover)}</b> days left`
      ...
      : r.orderFit === "short" && r.smart
        ? `A week draws <b>${spNum(r.weekNeed)}</b>; ${spKeeps(shown, plainAfter, r.plainBefore)}`
      : r.orderFit === "short"
...
function drawStock(){
  ...
  /* A missing plan is a finding even with plenty of stock. Check every row,
     including grades outside keep(), and retain ungraded findings such as
     unnamed factory lines. Tight but healthy shelves can still be all-clear. */
  const calm = all.length > 0
    && !all.some(r => stockUnplanned(r) || r.level === "critical" || r.level === "warn"
      || (!r.level && v.keep(r)));
...
function buildOrderChecklist(importRows, looseRows, sites, shops, imports, businesses){
  ...
  imports.forEach(r => {
    if(r.paused && !rows.some(x => x.kind === "Weekly imports" && x.site === r.s && x.item === r.item)) add("Weekly imports", r.s, r.item, null, null,
...
function orderChecklistText(rows, title){
  const out = [title, "Enter these settings in-game. Amounts are units, not boxes.",
    "Factory quantities assume full-rate production. Storage and transport capacity are not checked."];
  let group = null;
  rows.forEach(r => {
    ...
    const change = r.proposed === null ? "Review"
      ...
      : r.mode === "smart" ? `Smart Delivery stock ${r.current ?? "not set"} -> ${r.proposed} units`
...
function drawOrderChecklist(rows, factories){
  ...
  if(!orderMarkCache.has(cacheKey)){
    let saved = [];
          ...
          <details class="order-detail"><summary><span class="order-product">${attr(r.item)}</span>
            <span class="order-values">${r.proposed === null ? `<span class="order-review">${actionLabel(r)}</span>`
              ...
              : `<span class="order-before" aria-label="Saved setting ${r.current ?? "not set"}">${r.current === null ? "—" : r.current.toLocaleString()}</span>
                 <span class="order-arrow" aria-hidden="true">${icon("go")}</span><b class="order-after" aria-label="Proposed setting ${r.proposed}">${r.proposed.toLocaleString()}</b><small>${r.mode === "smart" ? "in stock" : r.kind === "Weekly imports" ? "/week" : "/day"}</small>`}</span>
  ...
  $("copyOrderChecklist").onclick = async () => {
    const pending = rows.filter(r => !marks.has(r.key));
...
function drawLogistics(){
  ...
  wireImportSet($("importPlan"), redraw);
}
/* A typed figure is kept on commit (Enter, or leaving the box) and redraws the
   ...
   or the suggestion itself, goes back to the suggestion. */
function wireImportSet(host, redraw){
  const again = id => {
    const box = [...host.querySelectorAll("input[data-imp]")].find(el => el.dataset.imp === id);
  ...
  host.querySelectorAll("input[data-imp]").forEach(box => box.onchange = () => {
    ...
    setTimeout(() => {
      const next = document.activeElement && document.activeElement.dataset
        ? document.activeElement.dataset.imp : null;
      redraw();
      if(next) again(next);
    });
  });
  host.querySelectorAll("[data-imp-reset]").forEach(button => button.onclick = () => {
    const id = button.dataset.impReset;
    impSetKeep(id, null);
    redraw();
    again(id);
  });
...
/* The product views paint as many rows as they always did (60 of what you
   sell, 40 of what you do not); "show all" only appears past that. */
const MARKET_TOP = {mine: 60, new: 40};
function drawMarket(){
  const m = D.market;
  ...
  if(!types && rows.length > limit) notes.push(showAllMarket
    ? `all ${rows.length} · <a class="link" href="#" id="marketMore">top ${limit}</a>`
...
/* changed for growth: one physical product is enough to plan — the hairdresser
   stocks a single haircare line beside its services. A type whose catalogue is
   services only stays out of the list; there is no line for it to feed. */
function planTypes(){
...
/* The company order for one material, in one pure function so it can be tested
   on its own: what the planned week needs beyond what the factories on the
   floor already eat, added to what the company already holds on active
   contracts, rounded up to the hundred the logistics manager takes. The
   selected range is the floor: the target never drops below a week of the
   range, so the range stays funded even when the factories on the floor
   already eat that much. The gap is what to change the contracts by. An order
   that does not exist yet counts as zero here — the caller keeps the "not
   ordered" wording and marks a positive target as a new order. Paused
   contracts never count; the game will not ship against them. */
function planOrder(week, baseline, ordered){
  const have = ordered === null || ordered === undefined ? 0 : ordered;
  const target = Math.ceil(Math.max(0, week, have + week - baseline) / 100) * 100;
  return {target, gap: target - have};
}
/* A line starts as the factories the player already runs: the machines on
   that product across every factory, and none for a product nobody makes, so
...
function drawPlan(){
  ...
  /* changed for growth: what the factories the player already runs eat of each
     material, read from the machines actually on the floor — never from the
     one-machine sketch this page seeds for a range nobody makes yet. Only lines
     of the selected range count, so an untouched range reproduces the company's
     standing orders (week equals baseline, the target keeps them) and a machine
  ...
  /* Per ingredient, by name: what is on order today (active contracts only, the
     paused pile kept as its own figure), what the real factories already eat,
     and the unit price where the company already buys it. planDraw() turns
     these into the Ingredients table's target, change and cash columns. */
  const meta = {};
  ...
  const lines = cat[planType].products.map(slug => {
    ...
    r.ingredients.forEach(i => {
      /* changed for growth: the recipe's input slug can name a variant of the
         product actually traded, so the contract lookup goes through the
         factory view's aliases before falling back to the slug itself; the
         price keeps the recipe slug first, the traded product second. */
      const src = sources[norm(i.slug)] || sources[i.slug];
      const unit = prices[i.slug] !== undefined ? prices[i.slug] : prices[norm(i.slug)];
  ...
  $("planNote").textContent = (bought ? `${plural(bought, "product")} of ${cat[planType].products.length} bought in` : "")
    + (svcs.length ? `${bought ? " · " : ""}also sells ${svcs.join(", ")} as services, which no line makes or stocks` : "");
  $("planBody").innerHTML = `
    <div class="planstats">
...
function drawFindLocation(){
  ...
  if(!vacant.length){
    ...
    text.textContent = D.premises ? "No vacant retail unit in the city right now."
      : "Free buildings ranked by demand, rivals and the building capacity you would get.";
    return;
  }
...
function drawOptimizeStaffing(){
  ...
  if(done.length){
    ...
    text.textContent = `Demand data complete at ${row.name}${
  ...
  if(!best){
    ...
    text.textContent = D.staffing
      ? "No shop has been measured long enough to plan a week for yet."
      : "Hours built from your customer count: registers, building capacity and the schedule each person asked for.";
  ...
  const alsoHire = !hiring && counts.hire
    ? `, and ${plural(spPlanPosts(row), "person", "people")} to hire` : "";
  ...
  badge.textContent = best.saves
    ? `\u2212${spRosterNow(row) - week} ENTRIES`
    : `${spPlanHours(row)} HOURS`;
  text.textContent = best.saves
    ? (measured
        ? `${row.name}: ${counts.now} entries become ${week}${alsoHire}.`
        : `${row.name}: ${counts.nowCover} cleaning and security entries become ${
    ...
    : (measured
        ? `${row.name}: a week of ${spPlanHours(row)} hours to set, in ${plural(lines, "entry", "entries")}${
          hiring ? `, every one of them waiting on a hire` : alsoHire}.`
        : `${row.name}: a week of ${spPlanHours(row)} cleaning and security hours to set, in ${plural(lines, "entry", "entries")}${
          hiring ? `, every one of them waiting on a hire` : alsoHire}.`);
  go.textContent = `Opens ${row.name} \u203a Staffing`;
  card.dataset.site = row.key;
}
...
/* --- the redesign's interactions ---------------------------------------------
   ...
   Supply — drawOrderChecklist, wireFlow:
     .up                  a suggested change to enter in-game; never changes
                          the saved setting shown in the table.
...
/* The host page (the in-browser board) closes a note when it opens its own
   menu over the same button, so both live on window. */
...
function applyFlow(){
  ...
  if(flowPickId && !nodes.some(m => m.dataset.id === flowPickId)) flowPickId = null;
...
function planDraw(){
  ...
  lines.forEach(tr => {
    ...
    if(coversEl) coversEl.innerHTML = (covers === null ? "—" : `${covers.toFixed(1)} shop${covers === 1 ? "" : "s"}`)
      + (wantWeek ? `<span class="sub">shops take ${fmtN(wantWeek)} · ${surplus >= 0
          ? chipHtml("ok", `+${fmtN(surplus)}`, "Surplus a week, for export")
          : chipHtml("bad", fmtN(surplus), "Short a week; this line needs more machines")}</span>` : "");
    let kit = []; try{ kit = JSON.parse(tr.dataset.kit || "[]"); }catch(e){}
    kit.forEach(k => kitCount[k] = (kitCount[k] || 0) + m);
    ...
    if(box){
      /* A dozen squares is as many as the row can carry; past that the count
         says the rest. Squares already there keep still; new ones pop. */
      const SQUARES = 12, shown = Math.min(m, SQUARES);
      const more = q("em", box); if(more) more.remove();
      Array.from(box.children).forEach((i, k) => { if(k >= shown) i.remove(); else i.classList.remove("new"); });
      while(box.children.length < shown){ const i = document.createElement("i"); i.className = "on new"; box.appendChild(i); }
      if(m > SQUARES){ const e = document.createElement("em"); e.textContent = `+${m - SQUARES}`; box.appendChild(e); }
    }
  });
  ...
  if(madeTile) madeTile.dataset.tip = wantWeek
    ...
    : shopsOwned ? `Demand is unknown: the ${shopsOwned} ${shopsOwned === 1 ? "shop sells" : "shops sell"} services or goods this planner cannot measure, so shop coverage and export surplus cannot be estimated`
  ...
  put("vKit", !machines ? "No machines set, so nothing is being made."
    : kitList.length ? `Machines to buy: ${kitList.map(([k, n]) => `${k} ×${n}`).join(", ")} · ${kitList.reduce((a, b) => a + b[1], 0)} in all`
    : "Nothing in this range is manufactured; it is all bought in.");
  /* the ingredient table: one row per material, summed over the lines that share it */
  const body = $("ingBody");
  if(body){
    // changed for growth: the host's data-ingmeta (JSON, name -> {tip, ordered,
    // active, paused, contracts, from, baseline, unit}) puts what is on order
    // today under each ingredient's name, and fills the target, change and
    // cash columns.
    ...
    body.innerHTML = Object.entries(ing).sort((a, b) => b[1].week - a[1].week).map(([name, r]) => {
      ...
      /* changed for growth: Company target and Change come from planOrder() —
         at least the range's own week, and more where the range adds beyond
         what real factories already eat, on top of every active contract.
         Per day, per week and cash stay the range's own
         figures. Nothing on order counts as zero for the target and keeps the
         "not ordered" wording; a positive target there is a new order to
         place, not a change. Paused contracts are shown apart and never
         counted. */
      ...
      `<td class="l"${i.tip ? ` data-tip="${attr(i.tip)}"` : ""}>${name}${i.from && !i.active ? ` ${chipHtml("warn", "paused", "Every contract for it is paused; none of it counts as ordered")}` : ""}</td>` +
    ...
    const foot = $("ingFoot");
    if(foot) foot.innerHTML = total ? `<tr><td class="l">Total</td><td></td><td>${fmtN(raw / 7)}</td><td>${fmtN(raw)}</td><td>${fmtN(targetSum)}</td><td></td><td></td><td class="cash">${cash ? fmt(cash) : ""}</td></tr>` : "";
    const note = $("ingNote");

...
.sp-rail .trk svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
...
.sp-m.q{background:none;box-sizing:border-box;border:1px dashed var(--warn);color:var(--warn);font:600 14px/1 "IBM Plex Mono",monospace}
.sp-m.q::before{display:none}
...
const spTok = s => "sp-it-" + String(s ?? "").toLowerCase().replace(/[^a-z0-9]+/g, "-");
...
/* A shift's own coordinates: the weekday, the station and the hour it starts.
   A plan that moves a shift simply drops its tick, which is the invalidation
   the block wants -- a tick means "this line is in the game", and a line that
   changed is not. */
const spTickId = s => `${s.d}-${s.s}-${s.f}`;
...
/* How much of *this* plan is typed. A tick left over from an older plan
   counts for nothing, so the ring never promises more than is there. */
const spTyped = (shifts, ticks) => (shifts || []).reduce(
  (n, s) => n + (ticks.has(spTickId(s)) ? 1 : 0), 0);
...
/* Stock standing still is a shelf on a shop and a line on a depot: the same
   finding, two blocks. */
const SP_EVIDENCE_BLOCK = {dead: kind => kind === "depot" ? "stock" : "shelves",
                           target: kind => kind === "depot" ? "stock" : "shelves"};
/* Groups whose evidence is one row of a table rather than a fixed mark: the
   finding's subject is the item's name, and every such row carries its token. */
const SP_EVIDENCE_ITEM = ["shortfall", "order", "paused", "dead", "target", "feed",
                          "outruns", "unplanned"];
const spFindRow = (a, b, kind) => {
  const ev = ALERT_EVIDENCE[a.group] || {};
  const block = SP_EVIDENCE_BLOCK[a.group] ? SP_EVIDENCE_BLOCK[a.group](kind) : ev.block;
...
function spRosterDay(c, wd, on){
  ...
  return `<div class="sp-day${on ? " on" : ""}" data-d="${wd}">${out}</div>`;
}
  ...
  return `<section class="sec rv" data-block="roster" id="sp-roster" data-readzone data-site="${attr(row.key)}">
    ...
    <div class="sp-ba">
      <div data-read="${attr(`Shifts to enter for the week: <b>${counts.now}</b> today${
        counts.fragments ? `, ${counts.fragments} of them two hours long` : ""}, against <b>${counts.plan}</b>`)}"><span class="lab">Shifts / week</span><div class="v">${
        spI("list")}<s>${counts.now}</s>${counts.plan}</div></div>
...
/* The seven days from today, left to right: a cell a day, filled while the
   ...
   the next delivery lands. A line nothing draws on has no cover to run out and
   sleeps instead. */
function spRail(cover, truck, held, dead){
  ...
  const end = truck === null ? c : held ? SP_RAIL_DAYS : truck;
  const cells = [...Array(SP_RAIL_DAYS).keys()].map(k =>
    `<i class="${k < c || (!held && truck !== null && k >= truck) ? "c" : k < end ? "d" : ""}"></i>`).join("");
  return `<span class="sp-rail">${cells}${truck === null ? ""
    : `<span class="trk${held ? " held" : ""}" style="--d:${truck}">${spIcon("truck")}</span>`}</span>`;
}
const spDays = today => `<span class="sp-days">${[...Array(SP_RAIL_DAYS).keys()].map(k =>
  `<b class="${k ? "" : "now"}">${SP_DAY_LETTER[((today || 0) + k) % 7]}</b>`).join("")}</span>`;
...
function spFeedRows(){
  ...
  (((D.supply || {}).factories || {}).sites || []).forEach(site =>
...
/* Who draws one item out of this depot, and how much of it a day. */
function spItemDraw(item){
  const sites = new Set();
  let perDay = 0;
  ((D.supply || {}).shops || []).forEach(r => {
    if(r.from === siteTab && r.item === item){ sites.add(r.s); perDay += r.sold || 0; }
  });
  (((D.supply || {}).factories || {}).sites || []).forEach(site =>
    (site.needs || []).forEach(n => {
      if(n.from === siteTab && n.item === item){ sites.add(site.s); perDay += n.perDay || 0; }
    }));
  return {sites: sites.size, perDay};
}
/* The factory that makes an item, where one of this company's does. */
function spMadeAt(item){
  for(const site of ((D.supply || {}).factories || {}).sites || [])
    if((site.lines || []).some(l => l.item === item)) return D.businesses[site.s] || null;
  return null;
}
...
function spStockRows(b){
  ...
  const named = new Set(imports.map(r => r.item));
  const rows = imports.map(r => {
    const away = Number.isFinite(r.arrives) && Number.isFinite(today) ? r.arrives - today : null;
    const truck = away !== null && away >= 0 && away < SP_RAIL_DAYS ? away : null;
    ...
    const order = r.paused ? spNum(r.weekly || r.lastWeek)
      : r.orderFit === "short" || r.orderFit === "tight"
        ...
        : spNum(r.weekly);
    const read = r.paused ? `Import <b>paused</b>; <b>${r.cover}</b> days left`
      ...
      : r.cover >= SP_RAIL_DAYS ? "Covered through the week"
      : `<b>${r.cover}</b> days on hand`;
    const draw = spItemDraw(r.item);
    return {item: r.item, hand: r.stock, draw: spNum(r.perDay),
            cover: r.paused ? null : r.cover, short: r.coverFit === "short",
            rail: spRail(r.cover, truck, r.paused, false), act, order,
            feeds: draw.sites, el, read};
  });
  (supply.idle || []).filter(r => r.s === siteTab && !named.has(r.item)).forEach(r => {
    named.add(r.item);
    rows.push({
      item: r.item, hand: r.stock, draw: r.perWeek ? spNum(r.perWeek / 7) : "—",
      rail: spRail(r.weeks === null ? 0 : r.weeks * 7, null, false, r.dead),
      act: "", order: "—", feeds: spItemDraw(r.item).sites, cover: null, short: false,
      el: [spTok(r.item), r.dead ? "dead" : "target"],
      read: r.dead ? "<b>Nothing draws</b> on these" : `<b>${r.weeks}</b> weeks on hand`,
    });
  });
  ...
  ((b && b.lines) || []).forEach(l => {
    if(named.has(l.item)) return;
    const draw = spItemDraw(l.item);
    if(!l.units && !draw.perDay) return;
    named.add(l.item);
    const made = spMadeAt(l.item);
    const cover = draw.perDay ? l.units / draw.perDay : 0;
    rows.push({
      item: l.item, hand: l.units, draw: draw.perDay ? spNum(draw.perDay) : "—",
      cover: draw.perDay ? cover : null, short: !!draw.perDay && cover < 1,
      rail: spRail(cover, null, false, !draw.perDay),
  ...
  rows.sort((x, y) => x.feeds === y.feeds ? 0 : y.feeds - x.feeds);
  return rows;
}
/* A factory's own row in the supply payload, or nothing when the lines could
   not be read at all. */
const spFactorySite = () => (((D.supply || {}).factories || {}).sites || [])
  .find(r => r.s === siteTab) || null;
/* One square a machine, filled by the share of the week somebody is posted to
   it: full unless the line lists it among its gaps. */
function spMachines(count, gaps, slots){
  const bySlot = new Map((gaps || []).map(g => [g.slot, g]));
  const posts = (slots && slots.length ? slots : [...Array(Math.max(count || 0, 0)).keys()].map(k => k + 1));
  return `<div class="sp-mach">${posts.map((slot, k) => {
    ...
    return `<span class="sp-m" style="--h:${Math.round(hours / SP_STAFF_HOURS * 100)}%" data-read="${attr(
      `Machine ${slot} · <b>${hours} of ${SP_STAFF_HOURS} h</b> rostered${
        gap && gap.off ? `: nobody on it ${spEsc(gap.off)}` : ""}`)}">${spIcon("gear")}</span>`;
  }).join("")}</div>`;
}
const SP_LINE_HEAD = `<div class="sp-line head"><span>Line</span><span>Machines</span><span></span>${
  ["Makes / day", "Ships / day", "On hand"].map(t => `<span class="n">${t}</span>`).join("")}</div>`;
...
function spLines(site){
  const where = r => `${spEsc(r.workstation)}${(r.slots || []).length
    ? ` · ${r.slots.map(s => `#${s}`).join(" ")}` : ""}`;
  let html = SP_LINE_HEAD;
  (site.lines || []).forEach(l => {
    const stop = !l.atRoster || (l.missing || []).length;
    const el = [spTok(l.item), l.piling ? "piling" : ""].filter(Boolean);
    html += `<div class="sp-line" data-line="${attr(spTok(l.item))}" data-el="${attr(el.join(" "))}">
      <div>${spEsc(l.item)}<span class="sub">${where(l)}</span></div>
      ${spMachines(l.machines, l.gaps, l.slots)}
      <div class="sp-belt${stop ? " stop" : ""}"></div>
      <div class="n">${spNum(l.atRoster)}<small>of ${spNum(l.makes)} rated</small></div>
      <div class="n">${spNum(l.ships)}</div>
      <div class="n">${l.piling
  ...
  (site.unnamed || []).forEach((u, k) => {
    ...
    const mach = `<div class="sp-mach">${(u.slots && u.slots.length ? u.slots
      : [...Array(Math.max(u.machines || 0, 0)).keys()].map(i => i + 1)).map(slot =>
      `<span class="sp-m ${idle ? "z" : "q"}" data-read="${attr(idle
        ? `Machine ${slot} · staffed and rented, <b>making nothing</b>`
        : `Machine ${slot} · running a recipe <b>the board cannot name</b>`)}">${idle ? "zz" : "?"}</span>`).join("")}</div>`;
    const pick = idle ? `<span class="quiet">No recipe</span>`
    ...
    html += `<div class="sp-line" data-line="sp-unnamed-${k}" data-el="${idle ? "unset" : "unnamed"}">
      ...
      <div class="sp-belt${idle ? " stop" : ""}"></div>
      <div class="n">${idle ? "0" : "—"}</div><div class="n">${idle ? "0" : "—"}</div><div class="n">${idle ? "0" : "—"}</div></div>`;
  });
...
function spInputs(site){
  return (site.needs || []).map(n => {
    ...
    const top = n.directImport
      ? (n.importWeekly === null || n.importWeekly === undefined ? "—" : `${spNum(n.importWeekly)}<small>/wk</small>`)
    ...
    return `<tr data-lines="${attr((n.lines || []).map(spTok).join(" "))}" data-el="${attr(el.join(" "))}" data-read="${
...
function drawSite(){
  ...
  if(dep){
    ...
    const onFloor = (b.lines || []).reduce((t, l) => t + (l.units || 0), 0);
    ...
    <div class="sstats rv" data-block="tiles" id="sp-tiles">
      ${spTile("Cost / day", fmt(spend), spCostBar(costs, null))}
      ${spTile("On the floor", `${onFloor.toLocaleString()}<small ${SMALL}>${
    ...
    <section class="sec rv" data-block="stock" id="sp-stock" data-readzone>
      ...
      ${rows.length ? `<div class="scrollx"><table>
        ...
        <tbody>${rows.map(r => `<tr data-el="${attr(r.el.join(" "))}" data-read="${attr(r.read)}">
          <td class="l">${spEsc(r.item)}</td><td>${r.hand.toLocaleString()}</td><td>${r.draw}</td>
          <td class="l">${r.rail}</td><td>${r.act}</td><td>${r.order}</td>
          <td>${r.feeds || "—"}</td></tr>`).join("")}</tbody></table></div>`
        : `<p class="quiet">Nothing is held or imported here yet.</p>`}
    ...
    <div class="duo sec" style="grid-template-columns:1fr 1fr">
      <section class="rv" data-block="feeds" id="sp-feeds">
        ...
        ${feedRows.length ? `<div class="sp-feeds">${feedRows.map(r => {
          ...
          return `<div class="sp-feed">${hoodHtml(site)}<span>${spEsc(shortName(site))}</span><span class="tr"><i style="--w:${
            (r.perDay / feedRows[0].perDay * 100).toFixed(0)}%"></i></span><span class="c">${spNum(r.perDay)}/day</span></div>`;
        }).join("")}</div>` : `<p class="quiet">Nothing draws on this depot.</p>`}
      </section>
  ...
  /* The factory's panel: the lines and what they eat. A machine runs only
     ...
     figure, with the rated output beside it. */
  if(fac){
    const site = spFactorySite() || {lines: [], unnamed: [], needs: [], machines: 0};
    const lines = site.lines || [];
    const running = lines.reduce((t, l) => t + (l.machines || 0), 0);
    const made = lines.reduce((t, l) => t + (l.atRoster || 0), 0);
    const rated = lines.reduce((t, l) => t + (l.makes || 0), 0);
    const ships = lines.reduce((t, l) => t + (l.ships || 0), 0);
    const spend = costs.reduce((t, [, v]) => t + v, 0);
    spBody = `
    <div class="sstats rv" data-block="tiles" id="sp-tiles">
      ${spTile("Machines", `${site.machines || 0}<small ${SMALL}>· ${running} running</small>`)}
      ${spTile("Made / day", `${spNum(made)}<small ${SMALL}>of ${spNum(rated)} rated</small>`,
        rated ? spMeter(made / rated * 100) : "")}
      ${spTile("Shipped / day", spNum(ships), made ? spMeter(ships / made * 100) : "")}
      ${spTile("Cost / day", fmt(spend), spCostBar(costs, null))}
    </div>
    <section class="sec rv" data-block="lines" id="sp-lines" data-readzone>
      ...
      ${lines.length || (site.unnamed || []).length ? `<div class="scrollx">${spLines(site)}</div>`
        : `<p class="quiet">No machines could be read here.</p>`}
    ...
    <section class="sec rv" data-block="inputs" id="sp-inputs" data-readzone>
      ...
      ${(site.needs || []).length ? `<div class="scrollx"><table>
        ...
        : `<p class="quiet">Nothing is drawn here yet.</p>`}
...
function buildOrderChecklist(importRows, looseRows, sites, shops, imports, businesses){
  ...
  const add = (kind, s, item, current, proposed, reason, source = null) => {
  ...
  importRows.forEach(d => d.rows.forEach(r => {
    if(r.paused && r.total > 0){
      ...
      add("Weekly imports", r.s, r.item, r.current ?? null, r.setTo, why);
    }
  }));
...
function drawLogistics(){
  ...
  Object.entries(depots).forEach(([si, items]) => {
    ...
    const rows = Object.values(items).map(r => {
      ...
      return {...r, s, otherWeek, total, current, fit, surplus, paused, pausedWeekly, stock: held(s, r.slug),
  ...
  const importRow = r => `<tr>
      ...
      <td data-now="${r.current || 0}" data-to="${r.setTo || 0}">${
        r.paused ? `${r.pausedWeekly.toLocaleString()} ${chipHtml("warn", "paused")}`
...
const wireSiteLines = once(() => {
  const light = (tr, on) => {
    ...
    if(!host) return;
    host.classList.toggle("dim", on);
    const want = (tr.dataset.lines || "").split(" ").filter(Boolean);
    $$(".sp-line", host).forEach(l => l.classList.toggle("lit", on && want.includes(l.dataset.line)));
  };
...
const wireRoster = once(() => {
  ...
  on("click", ".sp-daytabs a", (a, e) => {
    e.preventDefault();
    const s = sec(a);
    $$(".sp-daytabs a", s).forEach(x => x.classList.toggle("on", x === a));
    $$(".sp-day", s).forEach(d => d.classList.toggle("on", d.dataset.d === a.dataset.day));
  });

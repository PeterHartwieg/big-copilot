...
| The hand-built business dicts the Python tests pass to `_alerts()`: `def stub(` in `tests/test_site_panel_fields.py` (shared with `tests/test_hype_alerts.py`), and `def business(` in `tests/test_idle_week.py` (which `tests/idle_week_fixture.py` reuses for `tests/site_panel.test.cjs`). Neither has `series`. Most other tests build their businesses through the real `_business()`, which does; `tests/test_routed_supply.py` passes `SupplyOnly`, which a loop over the businesses sees as empty | Read a new field with `.get()` and a default, or add it to both fixtures, or those tests break | `tests/test_site_panel_fields.py`, `tests/test_hype_alerts.py`, `tests/test_idle_week.py`, `tests/site_panel.test.cjs` |
...
| `def _condense(` | *Only if* the kind merges and a field of its own must survive the merge. A merged row is built from a fixed set of keys (`level`, `site`, `siteKey`, `group`, `text`, `detail`, `worth`, `unit`, `id`, `always`, `ev`), taken from the worst row; any other field is dropped. Every row, merged or not, also loses `rank` and `subject`, and `always` once the materiality gate has used it | none |
...
| `tests/milestones.test.cjs` | Nothing, but mind its slice: it runs the source from `function drawGoals(){` to `/* Next moves: the Plan imports card` in a VM. A function declared in that range is harmless; a top-level statement there runs in the test, and moving or rewording the closing comment breaks the slice | that test |

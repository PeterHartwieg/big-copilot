...
that is `ba_buildings.json` (885 buildings with neighbourhood id, type, size code, floor area
...
the game's own (`ba:neighborhood_<id>`), as a building's `hood` is, so the page joins them
straight onto it.

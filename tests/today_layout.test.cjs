...
async function today(width, {debt = 0, profitSum7 = 70000} = {}) {
  ...
  await page.evaluate(([KEY, debt, profitSum7]) => {
    ...
    D = {
      ...
      alerts: [
        finding('feed', 'warn', 'HART. Gifts Clothing (Classic Expensive Female) arrives at 6,612/day against 11,520 needed while Import Hub holds 65,354; it isn\'t reaching the factory', null, ''),

      ...
      document.querySelectorAll(where).forEach(host => {
        ...
        for(let n = walk.nextNode(); n; n = walk.nextNode()){
          ...
          /* A finding's sentence is cut into headline and detail, so its
             brackets can open in one text node and close in the next. A
             message can hold another (a kind's name in Today's count line),
             so whole brackets go innermost first. */
          let text = n.textContent;
          for(let was = ''; was !== text;){ was = text; text = text.replace(/\[[^\[\]]*\]/g, ''); }
          const outside = text.replace(/^[^\[]*\]/, '').replace(/\[[^\]]*(\]|$)/g, '').replace(/[\d\s.,:;$%+\-–—×·/()!?%'"‹›…#]+/g, '');

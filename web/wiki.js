...
/* What the badges mean. The words are the reader's, never the file's.
   ...
   help, and says so. */
const WIKI_BADGE = {
  help: ["help", "Game help", "The game's own help page says so. A claim about the game, not a measurement of it."],
  asset: ["ok", "Checked", "Read against a file of another kind that the game ships — a shop layout it builds, or the city's building table — not only against the help text."],
  model: ["warn", "Big Copilot", "Our own reading, with the evidence named. This is the one that moves when the model does."],
  save: ["save", "Your save", "Only knowable once a save is open. Never shipped as a value."],
  gap: ["bad", "Gap", "The game files do not say. Recorded rather than filled."],
};
...
<div class="sechead wk-top">
  ...
  ${wikiWhy("Everything the game's own help menu knows, read straight out of the installed game and kept beside the board. "
    + "Type to find a page; the shelf keeps only the categories that still hold one. Every page is the game's own words, and "
    + "every fact the board adds carries a badge saying where it came from. Where the help gives a number, it is here as the "
    + "file writes it; where the help is silent, so is this.")}
...
<div class="wk-legend">
  ${legend}${wikiWhy("The five badges. Blue is the game's own help text; green is read against a file of another kind the game "
    + "ships, such as a shop it builds; amber is Big Copilot's reading with its evidence named; the dashed one only fills in "
    + "once a save is open; red is a gap the game files do not close.")}
</div>
...
<section class="sec">
  <div class="sechead"><h2>Yours</h2>
    ${wikiWhy("Nothing above changes when you load a save; this strip is the only part that does. It is matched to this page "
      + "by name, and stays empty rather than showing a number the save does not hold.")}
...
<section class="sec">
  <div class="sechead"><h2>To open</h2>
    ${wikiWhy("A filled square is something this business's own help page lists under what it requires to function, in its own "
      + "words. A hollow one is the board's suggestion, read off the help pages for the fixtures themselves — useful, but not "
      + "the game saying you must. Hover any line for which page it came from. Ticking is a scratch pad; nothing is stored.")}
...
<section class="sec">
  <div class="sechead"><h2>Sells</h2>
    ${wikiWhy("One card per product in the type's own range: what it goes on, where it comes from, who else sells it, all from "
      + "the help pages for the product and for the fixture. The capacity rides inside the fixture pill. A dashed red pill is a "
      + "door the help never opens — an absence, not a rule.")}
...
<section class="sec">
  <div class="sechead"><h2>Fits together</h2>
    ${wikiWhy("A product, the fixture it goes on, and where it comes from. Solid lines are goes-on; dashed lines are "
      + "comes-from and pass behind the middle column, because supply reaches a product without touching its shelf. "
      + "Pick anything to keep only its lines; pick it again, or press Escape, to let go.")}
...
<section class="sec">
  <div class="sechead"><h2>Make it</h2>
    ${wikiWhy("Each recipe's ingredients with the hourly rate its page gives and the importer that carries them, the "
      + "workstation, and what comes out. The rates are the game's own stated maximum per hour; the per-day chip is Big "
      + "Copilot's, and assumes a line that never stops.")}
...
function wikiSamplePlaces(){
  ...
  /* Anything this section would say about weekly limits is the extraction's own
     gap note, quoted, rather than lore typed in here — with the key of the page
     it read turned back into words, because a key belongs under Source and not
     in the middle of a section about where to shop. */
  const limits = (s.GAPS || []).find(g => /weekly|limit/i.test(g.what || ""));
  return `
<section class="sec">
  <div class="sechead"><h2>Where to go</h2>
    ${wikiWhy("Every address the help text gives is matched against the city table the board already ships, so each one carries "
      + "its neighbourhood and its pin opens the map on that building."
      + (limits ? ` ${wikiUnkey(limits.detail)}` : ""))}
...
function wikiPlanControl(){
  ...
  if(wikiCanPlan())
    slot.innerHTML = `<button type="button" class="link" data-wiki-plan
      data-tip="${attr(`Opens Plan a chain on Growth with the ${name} range already picked, against your own factories and orders.`)}"
      >Plan this range ${icon("chev")}</button>`;
  else
    slot.innerHTML = `<span class="quiet" data-tip="${attr(hasData()
      ? "This save's product catalogue does not carry this type, so the planner has nothing to select."
      : "Plan a chain works from your own factories and standing orders, so it needs a save.")}"
...
<section class="sec">
  <div class="sechead"><h2>Source</h2>
    ${wikiWhy("Everything above is read from the game's own help text and checked against the files the game ships. "
      + "The red dots are gaps: things the game does not say, recorded rather than filled.")}

...
const wikiTopicLabel = () => tt("wiki.topics.label", "Big Copilot topics");
...
/* What the badges mean. The words are the reader's, never the file's.
   ...
   help, and says so. Each is [class, label, tip]; the words are getters, so
   they are read in the language on screen. */
const WIKI_BADGE = {
  get help(){ return ["help", tt("wiki.badge.help", "Game help"), tt("wiki.badge.help.tip", "Stated in the game's help.")]; },
  get asset(){ return ["ok", tt("wiki.badge.asset", "Checked"), tt("wiki.badge.asset.tip", "Cross-checked with shop layouts or city data.")]; },
  get model(){ return ["warn", tt("wiki.badge.model", "Big Copilot"), tt("wiki.badge.model.tip", "Big Copilot's guidance or calculation.")]; },
  get save(){ return ["save", tt("wiki.badge.save", "Your save"), tt("wiki.badge.save.tip", "Read from your loaded save.")]; },
  get gap(){ return ["bad", tt("wiki.badge.gap", "Gap"), tt("wiki.badge.gap.tip", "Not specified in the game files.")]; },
};
...
const WIKI_CAT_LABEL = {get help_importers(){ return tt("wiki.cat.importers", "Wholesalers & importers"); }};
...
let wikiFailure = "";           // what to tell the reader when it is error (text, or a function giving it)
...
function wikiIndex(raw){
  ...
  const search = pages.map(p => ({
    ...
    /* Read when shown, so a change of language renames it. */
    get category(){ return catById.has(this.categoryId) ? wikiCatLabel(catById.get(this.categoryId)) : ""; },
  })).concat(topics.map(t => ({
    id: wikiTopicId(t.slug),
    title: String(t.title || t.slug),
    lower: String(t.title || t.slug).toLowerCase(),
    categoryId: WIKI_TOPIC_CAT,
    get category(){ return wikiTopicLabel(); },
...
/* A failure worded by the page: the words are said again at each draw, so a
   change of language reaches them. */
function wikiError(say){
  const error = new Error(say());
  error.wikiSay = say;
  return error;
}
function loadWikiData(){
  ...
  const got = embedded
    ...
    : fetch(`${WIKI_URL}?v=${window.LEDGER_BUILD || "1"}`).then(r => {
        if(!r.ok) throw wikiError(() => tt("wiki.fail.fetch", "the wiki content could not be fetched ({status})", {status: r.status}));
        return r.json();
      });
  got.then(raw => {
    if(!raw || raw.schemaVersion !== 1) throw wikiError(() => tt("wiki.fail.schema", "this build does not understand that wiki file"));
    if(!Array.isArray(raw.pages) || !raw.pages.length) throw wikiError(() => tt("wiki.fail.empty", "the wiki file carries no pages"));
    wikiData = wikiIndex(raw);
    wikiStatus = "ready";
    drawWiki();
  }).catch(error => {
    wikiStatus = "error";
    wikiFailure = (error && error.wikiSay) || (error && error.message) || String(error);
    drawWiki();
  });
}
...
function wikiFind(query){
  ...
  wikiData.search.forEach(entry => {
    /* A page named in the player's language answers to either name. */
    const at = [entry.lower, wikiName(entry.key, "").toLowerCase()].filter(Boolean)
      .map(t => t.indexOf(q)).filter(i => i >= 0).sort((x, y) => x - y)[0];
  ...
  hits.sort((a, b) => a.rank - b.rank || cmp(wikiName(a.entry.key, a.entry.title), wikiName(b.entry.key, b.entry.title)));
  return hits.map(h => h.entry);
}

/* --- the views ----------------------------------------------------------- */
/* The control that lifts a list's cap, by how many rows it would show. */
const wikiShowAllText = n => tt("wiki.list.showAll", {one: "Show all {n:,}", other: "Show all {n:,}"}, {n});
function wikiHome(){
...
function wikiCategoryView(){
  ...
  if(!cat) return wikiMissing(tt("wiki.missing.cat", "No category called “{id}”.", {id: wikiRoute.id}));
...
function wikiPageView(){
  ...
  if(!page) return wikiMissing(tt("wiki.missing.page", "No page called “{id}”. It may be one of the help links the game's own files leave dangling.", {id: wikiRoute.id}));
  ...
  return `
${wikiCrumb([{label: wikiCrumbHome(), href: "#wiki"},
...
function wikiSource(page){
  ...
  const sourceName = f => /BusinessLayouts/i.test(f.path || "") ? tt("wiki.src.layout", "Shipped shop layout")
    : /helpstructure/i.test(f.path || "") ? tt("wiki.src.menu", "Game help menu")
...
/* A game name in the language the footer's "Game names" picked (gnLocal() in
   the board), by the game's key; the catalogue's English where English is
   picked or the language has no word. Only what is shown: the articles stay
   English, and every match against the help's own words keeps the English
   name, which is why a record's `name` is never overwritten. */
const wikiName = (key, english) => (key && typeof gnLocal === "function" && gnLocal(key)) || english;
...
const WIKI_COPY = {
  get primaryTitle(){ return tt("wiki.copy.primaryTitle", "Sells"); },
  get secondaryTitle(){ return tt("wiki.copy.secondaryTitle", "Also sells"); },
  get servicesTitle(){ return tt("wiki.copy.servicesTitle", "Services"); },
  get primaryRecipesTitle(){ return tt("wiki.copy.primaryRecipesTitle", "Make it"); },
  get secondaryRecipesTitle(){ return tt("wiki.copy.secondaryRecipesTitle", "Also make"); },
  get dependenciesTitle(){ return tt("wiki.copy.dependenciesTitle", "Needs"); },
  get suppliersTitle(){ return tt("wiki.copy.suppliersTitle", "Where to go"); },
  get staffTitle(){ return tt("wiki.copy.staffTitle", "People"); },
  get fixturesTitle(){ return tt("wiki.copy.fixturesTitle", "Fixtures"); },
  get stockTitle(){ return tt("wiki.copy.stockTitle", "Stock"); },
  get setupTitle(){ return tt("wiki.copy.setupTitle", "To open"); },
  get graphTitle(){ return tt("wiki.copy.graphTitle", "Fits together"); },
  get sourceTitle(){ return tt("wiki.copy.sourceTitle", "Source"); },
  get roomTitle(){ return tt("wiki.copy.roomTitle", "The room"); },
  get automaticFee(){ return tt("wiki.copy.automaticFee", "Automatic"); },
  get automaticFeeTip(){ return tt("wiki.copy.automaticFeeTip", "Its own help page says this fee is charged automatically."); },
  get noRecipe(){ return tt("wiki.copy.noRecipe", "No recipe"); },
  get missingRecipe(){ return tt("wiki.copy.missingRecipe", "Recipe page missing"); },
  get missingRecipeTip(){ return tt("wiki.copy.missingRecipeTip", "A product's page names this recipe, but the recipe's own page is not in this build, so nothing about it can be read here."); },
  get noRequirements(){ return tt("wiki.copy.noRequirements", "None stated"); },
  get chooseProduct(){ return tt("wiki.copy.chooseProduct", "Choose a product"); },
  /* Where a claim would be, and the payload has none. */
  get noEquipment(){ return tt("wiki.copy.noEquipment", "no fixture named"); },
  get noOtherSellers(){ return tt("wiki.copy.noOtherSellers", "nobody else"); },
  get noListedSuppliers(){ return tt("wiki.copy.noListedSuppliers", "no vendor listed"); },
  get missingProduct(){ return tt("wiki.copy.missingProduct", "Product details unavailable"); },
  ...
  get listedRequirements(){ return tt("wiki.copy.listedRequirements", "Listed requirements"); },
  get linkedRequirements(){ return tt("wiki.copy.linkedRequirements", "Equipment and service requirements"); },
  get suggestedEquipment(){ return tt("wiki.copy.suggestedEquipment", "Additional equipment"); },
  get recruitmentTitle(){ return tt("wiki.copy.recruitmentTitle", "Recruitment"); },
  recruitmentHint: "",
  ...
  get sharedRecipe(){ return tt("wiki.copy.sharedRecipe", "Primary and secondary range"); },
  get recipeLink(){ return tt("wiki.copy.recipeLink", "Read the recipe"); },
  get originalHelp(){ return tt("wiki.copy.originalHelp", "The game's own words"); },
  get plannerLabel(){ return tt("wiki.copy.plannerLabel", "Plan this range"); },
  ...
  get alsoCarried(){ return tt("wiki.copy.alsoCarried", "also carried"); },
  get alsoCarriedTip(){ return tt("wiki.copy.alsoCarriedTip", "Not part of this type's own range: the help lists it among what the business also carries."); },
  get alsoOffered(){ return tt("wiki.copy.alsoOffered", "also offered"); },
  get alsoOfferedTip(){ return tt("wiki.copy.alsoOfferedTip", "Not part of this type's own range: the help lists it among what the business also offers."); },
  get primaryHint(){ return tt("wiki.copy.primaryHint", "Shelf numbers show storage capacity. A red dashed label means no supplier is listed in the help."); },
  get recipeHint(){ return tt("wiki.copy.recipeHint", "Rates are per workstation at full speed. Daily output assumes 24 hours without stopping."); },
  get setupHint(){ return tt("wiki.copy.setupHint", "Green squares mark listed requirements; hollow squares are suggestions. Tick items as you go; checkmarks reset on reload."); },
  get graphHint(){ return tt("wiki.copy.graphHint", "Select an item to highlight its shelves and suppliers. Select it again or press Escape to clear."); },
  get placesHint(){ return tt("wiki.copy.placesHint", "Select a map pin to locate a supplier or recruitment agency."); },
  get sourceHint(){ return tt("wiki.copy.sourceHint", "Red dots mark missing information. Expand the sections below to read the original help and sources."); },
  get fullDayHint(){ return tt("wiki.copy.fullDayHint", "Big Copilot's own reading: the page's maximum hourly rate times 24. That assumes the line runs all day, uninterrupted and at full rate — the help does not say what one achieves in practice, or what happens when an input runs out mid-hour."); },
...
/* The stand-in for the linked-requirements hint, which two kinds of row wear. */
const wikiLinkedHint = () => tt("wiki.setup.linked.hint", "Required when using the named equipment or service.");
...
<div class="wk-titlerow">
  ...
  <span class="chips">${wikiChip("help", tt("wiki.guide.chip.help", "game help"),
    ...
    placed ? wikiChip("asset", tt("wiki.guide.chip.counted", "fixtures counted in shipped shops"),
      tt("wiki.guide.chip.counted.tip", "Where the page says how many of a fixture a shipped shop holds, that count was read from the shop layouts the game installs — not from the help text. It says what those shops do, not what the game requires.")) : ""}</span>
</div>
...
function wikiGuideTiles(g, offers){
  ...
  <div class="kpi rv" data-tip="${attr(skills.length
    ...
    ? tt("wiki.tile.skills.tip", "{skills}.", {skills: wikiAnd(skills.map(k => wikiSkillName(b, k)))}) + (hiring.length
...
function wikiGuideSetup(g, page, offers, ctx){
  ...
  const requiredFixtures = required.filter(line => !productLine(line)).map((line, i) => {
    ...
    return item({
      ...
      shown: exact ? wikiFixName(one) : plain,
      html: exact ? null : wikiInline(line, ctx),
      meta: one
        ? `${serves(one)}${vendorCount(one.vendors)}`
      ...
      tip: one ? `${wikiFixTip(one)}${soldBy(one.vendors)}`
        : members.length ? members.map(k => `${wikiFixName(fixtures[k])}: ${wikiFixTip(fixtures[k])}`).join(" ")
        : tt("wiki.setup.req.tip", "Listed on this business's own page under what it requires to function."),
    });
  });
    ...
    wikiClauses(line).forEach(clause => {
      ...
      linkedFixtures.push(item({
        ...
        meta: one
          ? `${serves(one)}${vendorCount(one.vendors)}`
          : eachNamed(members),
        tip: tt("wiki.setup.linked.tip", "{fixture} requires this on its own help page: “{line}”.", {fixture: wikiFixName(fixtures[k]), line: wikiPlain(line)})
          + " " + wikiCopy("linkedRequirementsHint", wikiLinkedHint()),
      }));
  ...
  const optionalFixtures = shownSpare.map((k, i) => {
    ...
    return item({
      id: `fix-${k}`, req: false, name: fixtures[k].name, shown: wikiFixName(fixtures[k]),
  ...
  const own = required.filter(productLine).map((line, i) => item({
    ...
    tip: tt("wiki.setup.stock.tip", "In the business page's own words, under what it requires to function."),
  }));
  const linked = [
    ...needs.map((n, i) => item({
      id: `stock-need-${i}`, req: true, name: n.need, shown: n.shown, meta: tt("wiki.setup.need.meta", "the fixture's own page"), catch: n.missed,
      tip: tt("wiki.setup.need.tip", "{fixture} requires {need}, according to its help page.", {fixture: n.from, need: n.shown}) + " "
        + (n.missed ? tt("wiki.setup.need.missed", "The business page does not mention this requirement.") + " " : "")
        + wikiCopy("linkedRequirementsHint", wikiLinkedHint()),
    })),
  ];
...
/* --- what it sells -------------------------------------------------------- */
/* The words a card and the graph's lanes share. */
const wikiGoesOn = () => tt("wiki.card.goesOn", "Goes on");
...
function wikiGoodsCard(p, g){
  ...
  const goes = (p.fixtures || []).map(k => {
    ...
    return wikiPill(wikiFixName(f), caps.length ? wikiCapText(caps) : "", "", wikiFixTip(f));
  }).join("");
  const state = wikiWholesale(p);
  const from = [
    state === "yes"
      ? wikiPill(wikiAnyWholesaler(), wholesalers.length, "on",
          tt("wiki.card.wholesale.tip", "{wholesalers}. Its own help page names the wholesalers, and it is on their product list.", {wholesalers: wholesalers.join(", ")}))
      : state === "none"
      ? wikiPill(tt("wiki.card.wholesale.none", "No wholesaler listed"), "", "no",
          tt("wiki.card.wholesale.none.tip", "No wholesaler's page lists it and its own page names none. That is the help being silent, not a rule in the game: import it or make it, and check in-game before you count on it."))
      : wikiPill(tt("wiki.card.wholesale.unknown", "Wholesale not stated"), "", "unknown",
          tt("wiki.card.wholesale.unknown.tip", "The help does not say either way for this one, so neither does this page.")),
    ...(p.importers || []).map(k => {
...
function wikiGraphModel(primary, g){
  ...
  primary.forEach(p => {
    add("product", `p:${p.key}`, wikiShown(p), "");
    ...
    (p.importers || []).forEach(k => {
      ...
      if(!sup) return;
      add("source", `s:${k}`, sup.name, tt("wiki.graph.import", "{street} · import", {street: sup.street}));
    ...
    wikiRecipeKeys(p).forEach(key => {
      ...
      add("source", id, wikiName(recipe && recipe.workstationKey
        ? `ba:factoryworkstationtype_${recipe.workstationKey}` : null, station.name), tt("wiki.graph.factory", "your factory"));
      edges.push([`p:${p.key}`, id, "hop"]);
    });
  });
  if(anyWholesale){
    const n = (guide.WHOLESALERS || []).length;
    nodes.source.unshift({id: "s:wholesale", name: wikiAnyWholesaler(), sub: tt("wiki.graph.named", {one: "{n:,} named", other: "{n:,} named"}, {n})});
...
function wikiCrosscheckChip(primary){
  ...
  return wikiChip("help", tt("wiki.graph.both", "read both ways"),
    `${notes.join(" ")} ` + tt("wiki.graph.both.tip", "Both directions are the same help file agreeing with itself, not a second source."));
}
...
<section class="sec">
  ...
  <div class="wk-graph" id="wikiGraph" data-edges="${attr(JSON.stringify(model.edges))}">
    ...
    <div class="wk-lanes">
      ${lane(tt("wiki.graph.product", "Product"), model.nodes.product)}
      ${lane(wikiGoesOn(), model.nodes.fixture)}
      ${lane(wikiComesFrom(), model.nodes.source)}
    </div>
...
/* A workstation's note: the machines it is built from, and how many recipes
   it runs. */
function wikiStationTip(station){
  const machine = station.assembly ? wikiName(station.assemblyKey, station.assembly) : tt("wiki.recipe.machine", "An assembly machine");
  const line = (station.production || []).length
    ? tt("wiki.recipe.machine.with", "{machine} with {tools}", {machine,
...
/* --- where to go ----------------------------------------------------------- */
/* What a place is for, by the role it is gathered under. */
function wikiRoleWord(role){
  switch(role){
    case "fixtures": return tt("wiki.place.role.fixtures", "fixtures");
    case "products": return tt("wiki.place.role.products", "products");
    case "ingredients": return tt("wiki.place.role.ingredients", "ingredients");
    case "people": return tt("wiki.place.role.people", "people");
    case "machines": return tt("wiki.place.role.machines", "machines");
    default: return role;
  }
}
function wikiGuidePlaces(g){
  ...
  const rows = Object.keys(roles).map(key => {
    ...
    if(!sup) return "";
    const facts = [sup.size ? tt("wiki.place.size", "size {size}", {size: String(sup.size)}) : "",
      sup.area ? tt("wiki.place.area", "{n:,} m²", {n: sup.area}) : "",
      Number.isFinite(sup.traffic) ? tt("wiki.place.traffic", "traffic {n}", {n: sup.traffic}) : ""].filter(Boolean).join(" · ");
...
<section class="sec">
  <div class="sechead"><h2>${wikiText(wikiCopy("suppliersTitle"))}</h2>
    ...
    ${wholesalers.length ? `<span class="aside">${wikiChip("dim",
      ...
      tt("wiki.place.wholesalers.tip", "{names}. Named on the help's own wholesale page.", {names: wholesalers.map(w => w.name).join(", ")}))}</span>` : ""}</div>
...
/* A price the save does not carry. */
...
function wikiGuidePrices(g, offers){
  ...
  const hoodWords = h => h === UNKNOWN ? tt("wiki.prices.hood.unknown", "Unknown neighbourhood") : hoodName(h);
  ...
  /* The two price columns' headers, which each cell also carries for the
     narrow layout. */
  const colMine = tt("wiki.prices.col.mine", "Your configured price");
  const colMarket = tt("wiki.prices.col.market", "Lowest market price");
...
function wikiPaint(){
  ...
  if(say) say.textContent = wikiPicked ? tt("wiki.graph.say", {one: "{name} · {n} link", other: "{name} · {n} links"}, {name, n: links}) : "";
